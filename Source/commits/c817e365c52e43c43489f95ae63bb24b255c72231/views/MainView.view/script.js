...
//      controller.addMessageToActiveChat(message['author_name'], message['content'], message['mine'], message['created_at']);
  ...
  MainViewController.prototype._buildChatMessageContainer = function(author, authorAvatar, message, mine, createdAt) {
    ...
    if (authorAvatar !== '') {
      $('<img/>', {
        'class': this.options.presentation.floatLeftCls + ' diomsg-chat-message-author-avatar',
        src: authorAvatar
      }).appendTo(c1);
    } else {
      $('<span/>', {
        'class': this.options.presentation.noAuthorAvatarCls
      }).appendTo(c1);
    }
