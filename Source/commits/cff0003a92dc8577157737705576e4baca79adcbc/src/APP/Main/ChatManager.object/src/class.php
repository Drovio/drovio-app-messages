 ...
 * ID Generation
 * 
 * All the record creating methods of this class utilize an implementation of the IdGeneratorInterface in order to created IDs for the created records.
 * 
 * @version	9.0-1
 * @created	August 29, 2015, 11:23 (EEST)
 * @updated	September 28, 2015, 18:53 (EEST)
 */
class ChatManager {
	...
	/**
	 * See the Chat::isActive method for the definition of an active Chat.
	 * 
	 * @param	string	$chatId
	 * 		{description}
	 * 
	...
	/**
	 * Creates a copy of the "message" for each participant of the message's Chat.
	 * 
	 * See the Chat::getParticipants method for the definition of a Chat's participants.
	 * 
	 ...
	 * 
	 * @return	Chat
	 * 		The Chat created with the data included in the "request".
	 */
...
//		$nowString = $now->format(self::DATE_TIME_FORMAT);
	 ...
	 * 
	 * @return	ChatMessage
	 * 		The ChatMessage created with the data included in the "request".
	 */
	 ...
	 * 
	 * See Chat::isActive method for the definition of an active chat.
	 * 
	 ...
	 * 
	 * This method differs from the "deleteChat" method in that it just marks the chat as "deleted" for a participant instead of deleting it from the database.
	 * 
...
//				. ', DATE_FORMAT(created_at, "%m/%d/%Y %H:%i:%s") AS created_at_string'
	...
	public function findChatMessagesCopies($chatId, $myUserAccountId) {
		...
		$query = 'SELECT m.id AS "id"'
				...
				. ', chat_id'
//				. ', DATE_FORMAT(created_at, "%m/%d/%Y %H:%i:%s") AS created_at_string'
				. ', FROM_UNIXTIME(m.created_at, "' . static::$DB_DATE_FORMAT . '") AS created_at_string'
				. ', content'
	...
	public function findChatParticipant($accountId, $chatId) {
		...
		$avatar = $this->findChatParticipantAvatarFromAccount($accountId);
	...
	public function findChatParticipants($chatId, $deleted = -1) {
		...
		foreach ($results as $result) {
			...
			$avatar = $this->findChatParticipantAvatarFromAccount($result['account_id']);
	 ...
	 * 
	 * @return	string
	 * 		The participant avatar's URL.
	 * 		
	 * 		This value corresponds to the "profile_image_url" of the result of the account::info method.
	 */
	public function findChatParticipantAvatarFromAccount($accountId) {
		if ($this->chatParticipantNameCache->isPresent($accountId)) {
			$profileInfo = $this->chatParticipantNameCache->get($accountId);
			
			return isset($profileInfo['profile_image_url']) ? $profileInfo['profile_image_url'] : '';
		}
		
		$profileInfo = account::info($accountId);
		$avatarUrl = isset($profileInfo['profile_image_url']) ? $profileInfo['profile_image_url'] : '';
		$this->chatParticipantNameCache->add($accountId, $profileInfo);
	 ...
	 * 		
	 * 		This value corresponds to the "accountTitle" of the result of the account::info method.
	 */
...
//				. ', DATE_FORMAT(m.created_at, "' . static::$DB_DATE_FORMAT . '") AS "created_at"'
		...
		foreach ($results as $result) {
			...
			$authorAvatar = $this->findChatParticipantAvatarFromAccount($authorId);
	...
	/**
	 * See the Chat::isActive for the definition of an active chat.
	 * 
	 * @param	string	$chatId
	 * 		{description}
	 * 
	...
	/**
	 * Executes the "query" through the dbConnection::execute method.
	 * 
	 * Throws an \Exception if the result of the query execution is "false".
	 * 
	 * See method dbConnection::execute for more information on the parameters of this method.
	 * 
	 * @param	string	$query
	 * 		{description}
	 * 
	 ...
	 * 
	 * @return	mixed
	 * 		The result of the dbConnection::execute method.
	 * 
	 * @throws	\Exception
	 */
	 ...
	 * 
	 * @return	ChatMessage[]
	 * 		An array of ChatMessage instances with all of the classes properties set. That includes the following extra properties:
	 * 		  - author's avatar,
	 * 		  - author's name and
	 * 		  - the "mine" marker.
	 */
