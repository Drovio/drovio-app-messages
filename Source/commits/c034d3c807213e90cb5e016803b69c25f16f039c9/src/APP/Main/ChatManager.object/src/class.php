...
class ChatManager {
	...
	public function createMessageCopies(ChatMessage $message) {
		...
		$queryValuesFormat = "(NULL, %d, %d, '%s', NULL)";
		$queryValues = array();
		...
		$query = 'INSERT INTO ' . static::$DB_TABLE_MESSAGE_OWNER . ' VALUES '
			. $queryValuesString;
...
//				: $this->findChatParticipantNameFromAccount($result['author_id']);
	...
	private function saveParticipantsOfNewChat(Chat $chat) {
		...
		$queryDataFormat = "(NULL, %d, %d, %d, %d, %d)";
		
		$queryDataArr = array();
		foreach ($chat->getParticipants() as $participantId) {
			$active = $participantId === $ownerId;
			$queryData[] = sprintf($queryDataFormat, $chatId, $participantId, 1, $active, 0);
		}
