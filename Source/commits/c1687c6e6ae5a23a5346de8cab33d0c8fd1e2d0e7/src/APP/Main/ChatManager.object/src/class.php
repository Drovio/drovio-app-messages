...
class ChatManager {
	...
	public function findPendingMessagesOfChatParticipant($participantId, $chatId) {
		$query = 'SELECT pm.id'
				...
				. ', m.author_id'
				. ', m.content'
				. ', DATE_FORMAT(m.created_at, "' . static::$DB_DATE_FORMAT . '") AS "m.created_at"'
...
throw new \Exception('Query: ' . $query);				
		...
		foreach ($results as $result) {
			$message = PendingMessage::newWithId($result['pm.id'])
				...
				->withAuthor($result['m.author_id'])
				->withContent($result['m.content'])
				->withCreatedAt($result['m.created_at']);
		}
