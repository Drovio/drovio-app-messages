...
var MainViewController = (function($) {
  ...
  function MainViewController(options) {
    ...
    this.options = $.extend({
      ...
      pendingMessagesPollTimeInterval: 1500,
...
//    this.addMessageToActiveChat(this.options.myChatMessagesAuthorLabel, messageField.val());
  ...
  MainViewController.prototype.onNewChat = function(event, response) {
    ...
    var decodedResponse = $.parseJSON(response),
      firstFreeChatList = this._getFirstFreeChatList(),
      chatId = decodedResponse['chatId'];

    firstFreeChatList.attr('id', this.options.chatListCls + '-' + chatId);
    this._setActiveChatList(firstFreeChatList);


    this._startTimerForChatPendingMessages(chatId);
  };
  ...
  MainViewController.prototype.onNewMessage = function(event, response) {
    ...
    var chatId = this._getNewMessageFormChatIdField().val();
//    this._pollForPendingMessagesOfChat(chatId);
  ...
  MainViewController.prototype._startTimerForChatPendingMessages = function(chatId) {
    var intervalId = setInterval(
      $.proxy(this._pollForPendingMessagesOfChat, this, chatId),
      this.options.pendingMessagesPollTimeInterval
    );
  };
