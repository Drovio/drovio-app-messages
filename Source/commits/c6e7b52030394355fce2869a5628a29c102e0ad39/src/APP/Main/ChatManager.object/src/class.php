...
class ChatManager {
	...
	public function createNewChat(NewChatRequest $request) {		
		try {
			$team = ChatParticipantTeam::newWithId($request->getTeamId());
			...
			$chat = Chat::newWithAll($owner, $recipients, $team);
	...
	public function createNewMessage(NewChatMessageRequest $request) {
		...
		$newMessage = ChatMessage::newWithId($newMessageId)
			->withContent($content);
	...
	private function saveNewChat(Chat $chat) {
		...
		$query = 'INSERT INTO ' . static::$DB_TABLE_CHATS . ' VALUES'
			. ' (NULL, ' . $owner->getAccountId() . ', ' . $team->getId() . ');';
		$query .= 'SELECT LAST_INSERT_ID() as id;';
		$dbc = $this->dbConnBuilder->getConnection();
		$resultSet = $dbc->execute($query);
